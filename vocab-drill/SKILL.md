...
**先查待测的词有没有现成词卡**（`--list` 的「有卡/无卡」列）：已带卡的——如从
english-reading-exercises 导入的，例句是文章原句——直接用 `--due --json` 取回原卡进
session，**不重新生成**（2026-08 用户定夺：原文例句优先于 quirky 例句）；只有无卡的词才现造。
...
- **复习会话渲染**：渲染到期复习的学习页时，cards 必须用 `--due --json` 取回的
  **原词卡**（例句是记忆锚点，现场重造等于换锚点，见第 2 步）。只有**无词卡**的新词才现造卡——
带卡的新词（如阅读导入的）同样取回原卡用。
